...
桌面列表中的停止并暂停会持久化禁用该目标，停止当前录制，后续值守不会自动重启；恢复会重新启用目标，并由下一轮值守处理。

桌面端使用 FFmpeg 处理可用的直播流。平台解析可能受网络、Cookie、直播间权限和上游解析引擎版本影响。

## 桌面预览与已知限制

预览仅支持 Windows 和 Linux，Android 暂不支持预览。桌面端同时只能预览一个直播间，并且默认静音；停止预览只会关闭预览，不影响录制任务。

每次预览都会通过 PyAV 建立独立连接并解码画面，不复用或控制 FFmpeg 录制进程，因此预览中断不影响录制。独立连接会额外占用网络带宽，也可能受到直播源并发连接限制。预览依赖 PyAV 及其 FFmpeg 原生库，能否显示仍取决于直播协议、编码格式和当前网络环境。
...
- `[recorder]`：保存目录、输出格式、画质、分段、转换与最低剩余磁盘空间选项。
- `[monitor]`：检查间隔与最大并发数。
- `[proxy]`：代理开关和地址。
...
需要 Python 3.10–3.13。桌面录制要求系统能够使用 FFmpeg，桌面预览还依赖 `requirements-gui.txt` 中严格固定版本的 PyAV。
...
Windows 和 Linux 桌面构建输出到 `dist/Lubo`，Android debug APK 输出到 `dist/android/Lubo-android-debug.apk`。桌面脚本使用项目内的 PyInstaller hooks 收集 PyAV 子模块和动态库，并在构建结束前确认产物包含当前平台的 PyAV 原生扩展。
...
`alpha.3` 源码版本为 `0.2.0a3`，对应 Release 标签 `v0.2.0-alpha.3`。该 Release 的永久产物为：
...
`SHA256SUMS` 文件记录三个可执行产物的 SHA-256 checksum。下载后应在运行或安装前计算文件的 SHA-256，并与该文件中对应条目核对；校验不一致时不要使用该产物。
